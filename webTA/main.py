...
# Clean, simplified CSS for consistent light mode styling
st.markdown("""
<style>
    ...
    /* Text colors */
    body, .stApp, div, p, h1, h2, h3, h4, h5, h6, span, li, a {
        color: #333 !important;
    }
    
    /* Content sections with translucent backgrounds */
    .stMarkdown, [data-testid="stExpander"] {
        background-color: transparent !important;
        padding: 0.5rem !important;
    }
    
    /* Charts and tables with translucent backgrounds */
    .stPlotlyChart {
...
# load main CSS file with forced light mode
...
def main_home():
    ...
    try:
        ...
        with col2:
            wordcloud_path = os.path.join(BASE_DIR, "assets", "wordcloud.png")
            st.image(
                wordcloud_path,
        ...
        image_path = os.path.join(BASE_DIR, "assets", "wordcloud.png")
        st.info(f"""
    ...
    data_path = os.path.join(BASE_DIR, "sampled_sentiment_data_balanced.xlsx")
    try:
    ...
    fig.update_layout(
        ...
        font=dict(size=12),
        margin=dict(l=40, r=40, t=50, b=40),  # Adjusted margins
        autosize=True  # Ensure responsive sizing
    )
    balanced_data = {
        'Sentiment': ['negative', 'neutral', 'positive'],
    ...
    # Add container styling to ensure charts fit properly
    st.markdown("""
    <div style='padding: 10px 0px;'>
        <h4 style='color: #1565C0; margin-top: 0;'>Sentiment Distribution Charts</h4>
    </div>
    """, unsafe_allow_html=True)
...
def main_analysis():
    ...
    template_file = os.path.join(BASE_DIR, "cleaned_data.xlsx")
    st.sidebar.download_button(
        label="Click here to download excel for analysis",
    ...
    if uploaded_file is not None:
        try:
            ...
            with tabs[-2]:
                ...
                cm_hard_styled = cm_hard_df.style.background_gradient(cmap='Blues').set_table_styles([
                    {'selector': 'th', 'props': [('font-size', '0.85rem'), ('padding', '4px')]},
                    {'selector': 'td', 'props': [('font-size', '0.85rem'), ('padding', '4px')]}
                ])
                ...
                cm_soft_styled = cm_soft_df.style.background_gradient(cmap='Blues').set_table_styles([
                    {'selector': 'th', 'props': [('font-size', '0.85rem'), ('padding', '4px')]},
                    {'selector': 'td', 'props': [('font-size', '0.85rem'), ('padding', '4px')]}
                ])
                ...
                with cm_col1:
                    st.markdown('<div class="sentiment-table-container">', unsafe_allow_html=True)
